...
def my_svd(Rstim, full_matrices=False):
    return np.linalg.svd(Rstim, full_matrices=False)



def ridge_corr(Rstim, Pstim, Rresp, Presp, alphas, U=None, S=None, Vh=None, normalpha=False, corrmin=0.2,
               singcutoff=1e-10, use_corr=True, logger=ridge_logger, return_svd=False):
    ...
    try:
        if U is None or S is None or Vh is None:
            U,S,Vh = my_svd(Rstim, full_matrices=False)
        else:
            print("SVD matrices were calculated before")
    except timeout_decorator.timeout_decorator.TimeoutError:
        print("SVD timeout moving to next layer")
        return None, None, None, None
        # print("NORMAL SVD FAILED, trying more robust dgesvd..")
        # from text.regression.svd_dgesvd import svd_dgesvd
        # U,S,Vh = svd_dgesvd(Rstim, full_matrices=False)
    # finally:
    #     print("All SVDs failed moving to next layer")
